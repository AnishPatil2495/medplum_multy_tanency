...
export class AppointmentController {
  ...
  create = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const appointment = await this.appointmentService.create(dto, req.medplumClient, tenant.id);
  ...
  delete = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const tenant = requireTenant(req);
      if (!req.medplumClient) throw AppError.internal('Medplum client not initialized');
      await this.appointmentService.delete(req.params['id']!, req.medplumClient, tenant.id);
      sendNoContent(res);
    } catch (err) {
      next(err);
    }
  ...
  search = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const bundle = await this.appointmentService.search(params, req.medplumClient);
